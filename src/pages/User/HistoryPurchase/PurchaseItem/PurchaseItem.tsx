    ...
    return (
        <div className='bg-white px-6 py-4 rounded'>
            ...
            <div className='flex justify-between pb-4 border-gray-100 border-b-[1px]'>
                ...
                {/* Status */}
                <div className='font-semibold text-primary uppercase'>{getDisplayStatus(purchase.status)}</div>
            </div>
            {/* Product info */}
            <div className='gap-x-2 grid grid-cols-12 py-4 border-gray-100 border-b-[1px]'>
                {/* Image */}
                <div className='relative col-span-1 pt-[100%]'>
                    <img className='absolute inset-0' src={purchase.product.image} alt='' />
                </div>
                {/* Detail info */}
                <div className='flex justify-between items-center col-span-11'>
                    <div className='flex-1'>
                        <h2 className='max-w-[60%] text-base capitalize'>{purchase.product.name}</h2>
                        <span>x1</span>
                    </div>
